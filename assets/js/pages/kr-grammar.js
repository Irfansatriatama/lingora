/**
 * Lingora - Korean Grammar Page JS (Fase 21.4)
 * Module: kr-grammar | Data: KrGrammarData
 * Accordion per kategori, search, filter TOPIK, tandai hafal
 */
document.addEventListener('DOMContentLoaded', () => {
  if (!Router.guard()) return;
  App.init('kr-grammar');

  const MODULE_ID    = 'kr-grammar';
  ...
  function updateHeader() {
    ...
    const total   = KrGrammarData.getAll().length;
  ...
  function renderCategoryTabs() {
    ...
    const allBtn    = `<button class="theme-tab active" data-cat="all">Semua (${KrGrammarData.getAll().length})</button>`;
    const catBtns   = KrGrammarData.getCategories().map(c => {
      const count = KrGrammarData.getByCategory(c.id).length;
  ...
  function getFiltered() {
    let data;
    if (searchQuery) {
      data = KrGrammarData.getAll().filter(p =>
        p.pattern.toLowerCase().includes(searchQuery) ||
        p.meaning.toLowerCase().includes(searchQuery) ||
        (p.explanation && p.explanation.toLowerCase().includes(searchQuery))
      );
    } else {
      data = activeCategory === 'all'
        ? KrGrammarData.getAll()
        : KrGrammarData.getByCategory(activeCategory);
    }
  ...
  function renderList() {
    ...
    if (data.length === 0) {
      listEl.innerHTML = `<div class="zh-empty"><div class="zh-empty-icon">문</div><p>Tidak ada pola ditemukan.</p></div>`;
      return;
    }
    ...
    listEl.innerHTML = data.map(p => {
      ...
      const catDef     = KrGrammarData.getCategories().find(c => c.id === p.category);
        ...
        <div class="grammar-card${isExpanded ? ' expanded' : ''}${isLearned ? ' learned' : ''}" data-id="${p.id}">
          <div class="grammar-card-header">
            <div class="grammar-card-main">
              <div class="grammar-pattern kr-pattern">${p.pattern}</div>
              <div class="grammar-meaning">${p.meaning}</div>
            </div>
            <div class="grammar-card-meta">
              <span class="badge ${levelClass(p.level)}" style="font-size:0.65rem">${levelLabel(p.level)}</span>
              <span class="kr-cat-badge">${catIcon}</span>
            ...
            <div class="grammar-examples">
              ...
              ${p.examples.map(ex => `
                <div class="grammar-example-item">
                  <div class="grammar-ex-jp kr-ex-kr">${ex.kr}</div>
                  <div class="grammar-ex-romaji kr-ex-roman">${ex.roman}</div>
      ...
      btn.addEventListener('click', e => {
        ...
        if (wasLearned) {
          ...
          if (typeof XPSystem !== 'undefined') {
            XPSystem.addXP(user.id, 'learn_item', 5, 'Hafal grammar Korea: ' + id);
          }
        }
        updateHeader();
        renderList();
