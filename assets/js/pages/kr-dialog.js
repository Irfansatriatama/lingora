...
(function () {
  ...
  if (user && user.settings) {
    showRomanization = user.settings.showRomanization !== false;
  }
  ...
  function init() {
    ...
    renderGrid();
    bindFilterBar();
    ChallengeSystem && ChallengeSystem.onModuleVisit && ChallengeSystem.onModuleVisit('kr-dialog');
  }

  // ── Filter Bar ──────────────────────────────────────────
  function renderFilterBar() {
    const levels = ['all', 'TOPIK I', 'TOPIK I-II'];
  ...
  function renderGrid() {
    const data = levelFilter === 'all'
      ? KR_DIALOGS
      : KR_DIALOGS.filter(d => d.level === levelFilter);
  ...
  function openDialog(id) {
    activeDialog = KR_DIALOGS.find(d => d.id === id);
    ...
    renderViewer();
    ChallengeSystem && ChallengeSystem.onLearnItem && ChallengeSystem.onLearnItem('kr-dialog');
    Progress.markLearned && Progress.markLearned('kr-dialog', id);
    XPSystem && XPSystem.addXP && XPSystem.addXP(user ? user.id : null, 'learn_item', 5, 'Buka dialog KR');
  }
  ...
  function renderViewer() {
    ...
    const audioSupported = window.AudioEngine && AudioEngine.isSupported && AudioEngine.isSupported();
    ...
    viewerSection.innerHTML = `
      ...
      <div class="dialog-controls">
        <button class="dialog-toggle-btn ${showRomanization ? 'on' : ''}" id="toggle-roman">로마자 Romanisasi</button>
      ...
      <div class="dialog-lines" id="dialog-lines">
        ${d.lines.map((line, i) => renderLine(line, i, audioSupported)).join('')}
      </div>

      ${renderVocab(d.vocab, audioSupported)}
    `;

    // Bind controls
    document.getElementById('dialog-back').addEventListener('click', closeViewer);
    document.getElementById('toggle-roman').addEventListener('click', toggleRomanization);
    ...
    // Apply initial romanization state
    if (!showRomanization) {
  ...
  function renderLine(line, index, audioSupported) {
    const audioBtn = audioSupported
      ? `<button class="dialog-line-audio" data-text="${line.kr}" title="Dengar">🔊</button>`
      : '';
    return `
      <div class="dialog-line speaker-${line.speaker.toLowerCase()}" data-index="${index}">
        ...
        <div class="dialog-line-content">
          <div class="dialog-line-main kr-word">${line.kr}</div>
          <div class="dialog-line-sub dialog-line-pinyin kr-romanization">${line.roman}</div>
  ...
  function renderVocab(vocab, audioSupported) {
    ...
    return `
      <div class="dialog-vocab-section">
        ...
        <div class="dialog-vocab-grid">
          ${vocab.map(v => `
            <div class="dialog-vocab-item" data-text="${v.kr}" title="Klik untuk dengar">
              <div class="dialog-vocab-jp kr-word">${v.kr}</div>
              <div class="dialog-vocab-reading kr-romanization">${v.roman}</div>
  ...
  function stepNext() {
    ...
    if (window.AudioEngine && AudioEngine.isSupported && AudioEngine.isSupported()) {
      const text = activeDialog.lines[currentHighlight].kr;
      setTimeout(() => AudioEngine.speakKR(text), 200);
    }
  ...
  function toggleRomanization() {
    ...
    const btn = document.getElementById('toggle-roman');
    btn.classList.toggle('on', showRomanization);
    viewerSection.querySelectorAll('.dialog-line-sub').forEach(el => {
      el.style.display = showRomanization ? '' : 'none';
    });
  }
